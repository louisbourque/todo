...
var app=app||{};!function(){"use strict";app.Action=Backbone.Model.extend({defaults:{title:"",note:"",category:"",selected:!1,completed:!1},select:function(e){this.save({selected:e})},toggleCompleted:function(){this.save({completed:!this.get("completed")}),app.actions.trigger("toggleCompleted")}})}();
var app=app||{};!function(){"use strict";app.Category=Backbone.Model.extend({defaults:{title:"",selected:!1,completedCount:0,remainingCount:0},select:function(e){this.save({selected:e})}})}();
var app=app||{};!function(){"use strict";var e=Backbone.Collection.extend({model:app.Action,localStorage:new Backbone.LocalStorage("todo-timetrack-actions"),comparator:"order",nextOrder:function(){return this.length?this.last().get("order")+1:1},selected:function(){return this.where({selected:!0})},completed:function(){return this.where({completed:!0})},remaining:function(){return this.where({completed:!1})},completedByCategory:function(e){return this.where({completed:!0,category:e})},remainingByCategory:function(e){return this.where({completed:!1,category:e})},actionsByCategory:function(e){return this.where({category:e})}});app.actions=new e}();
var app=app||{};!function(){"use strict";var e=Backbone.Collection.extend({model:app.Category,localStorage:new Backbone.LocalStorage("todo-timetrack-categories"),comparator:"order",nextOrder:function(){return this.length?this.last().get("order")+1:1},selected:function(){return this.where({selected:!0})}});app.categories=new e}();
var app=app||{};!function(e){"use strict";app.ActionDetailsView=Backbone.View.extend({el:".action-details",template:_.template(e("#action-details-template").html()),events:{"click .toggle":"toggleCompleted","click .destroy":"clear","keyup .edit":"handleKeyPress","blur #action-title":"updateNavigation"},selectedAction:{},initialize:function(){this.$detailsDiv=this.$(".action-details-div"),this.listenTo(app.actions,"remove",this.render),this.listenTo(app.actions,"filter",this.render),this.listenTo(app.actions,"toggleCompleted",this.render),this.listenTo(app.actions,"actionSeletected",this.render),this.render()},render:function(){if(app.selectedCategoryID&&app.selectedActionID)if(this.selectedAction=app.actions.get(app.selectedActionID),!this.selectedAction.get("visible")||"category-all"!==app.selectedCategoryID&&this.selectedAction.get("category")!==app.selectedCategoryID)this.$el.hide();else{this.$el.show();var e=app.categories.get(this.selectedAction.get("category"));this.$detailsDiv.html(this.template({action:this.selectedAction,category:e.get("title")}))}else this.$el.hide();this.$title=this.$(".action-details-div #action-title")},toggleCompleted:function(){this.selectedAction.toggleCompleted()},clear:function(){var t=this.selectedAction;e(function(){e("#dialog-confirm #dialog-message").html("Are you sure you want to permanently delete this action?"),e("#dialog-confirm").dialog({title:"Delete Action",resizable:!1,height:"auto",width:400,modal:!0,buttons:{"Delete Action":function(){t.destroy(),e(this).dialog("close")},Cancel:function(){e(this).dialog("close")}}})})},handleKeyPress:function(t){t.which===ESC_KEY?e(t.target).val(t.target.defaultValue):t.which===ENTER_KEY&&"INPUT"===t.target.nodeName.toUpperCase()&&e(t.target).blur(),this.close(t.target)},close:function(t){var i=e(t),a=i.val(),o=a.trim();o&&("action-title"===t.id?this.selectedAction.save({title:o}):"action-note"===t.id&&this.selectedAction.save({note:o}))},updateNavigation:function(){app.AreaRouter.navigate(app.selectedAreaID+"/p"+app.selectedProjectID+"/a"+this.selectedAction.id+"/f"+app.ActionStatusFilter,{trigger:!0})}})}(jQuery);
var app=app||{};!function(t){"use strict";app.ActionView=Backbone.View.extend({tagName:"li",template:_.template(t("#action-template").html()),events:{"click label":"select","click .destroy":"clear","click .toggle":"toggleCompleted"},initialize:function(){this.listenTo(this.model,"change",this.render),this.listenTo(this.model,"destroy",this.remove),this.listenTo(this.model,"visible",this.toggleVisible)},render:function(){if(void 0===this.model.changed.id)return this.$el.html(this.template(this.model.toJSON())),this.$el.toggleClass("completed",this.model.get("completed")),this.$el.toggleClass("hidden",!this.model.get("visible")),this.$input=this.$(".edit"),this.$el.addClass("draggable-action"),app.selectedActionID==this.model.id&&(this.$el.addClass("selected"),app.actions.trigger("new-action-selected")),this},select:function(){app.dragged||(app.selectedActionID!=this.model.id?this.updateNavigation(this):this.updateNavigationNoAction(this))},toggleVisible:function(){this.model.set("visible",!this.isHidden())},toggleCompleted:function(){this.model.toggleCompleted()},isHidden:function(){return!!app.selectedCategoryID&&this.model.get("category")!=app.selectedCategoryID&&"category-all"!=app.selectedCategoryID||"all"!=app.ActionStatusFilter&&this.model.get("completed")&&"active"===app.ActionStatusFilter||!this.model.get("completed")&&"completed"===app.ActionStatusFilter},edit:function(){this.$el.addClass("editing"),this.$input.focus()},clear:function(){var e=this.model;t(function(){t("#dialog-confirm #dialog-message").html("Are you sure you want to permanently delete this action?"),t("#dialog-confirm").dialog({title:"Delete Action",resizable:!1,height:"auto",width:400,modal:!0,buttons:{"Delete Action":function(){e.destroy(),t(this).dialog("close")},Cancel:function(){t(this).dialog("close")}}})})},updateNavigation:function(t){app.AreaRouter.navigate(app.selectedCategoryID+"/a"+t.model.id+"/f"+app.ActionStatusFilter,{trigger:!0})},updateNavigationNoAction:function(t){app.AreaRouter.navigate(app.selectedCategoryID+"/f"+app.ActionStatusFilter,{trigger:!0})}})}(jQuery);
var app=app||{};!function(e){"use strict";app.ActionListView=Backbone.View.extend({el:".actions",statusTemplate:_.template(e("#status-filter-template").html()),events:{"keypress .new-action":"createOnEnter","click .clear-completed":"clearCompleted","click .toggle-all":"toggleAllComplete","click .filter-all":"setFilterAll","click .filter-active":"setFilterActive","click .filter-completed":"setFilterCompleted"},initialize:function(){this.$input=this.$(".new-action"),this.$statusfilter=this.$(".status-filter"),this.$list=e(".action-list"),this.allCheckbox=this.$(".toggle-all")[0],this.$el.removeClass("hidden"),this.listenTo(app.actions,"add",this.addOne),this.listenTo(app.actions,"reset",this.addAll),this.listenTo(app.actions,"remove",this.render),this.listenTo(app.actions,"filter",this.render),this.listenTo(app.actions,"toggleCompleted",this.render),this.listenTo(app.actions,"updateOrder",this.updateOrder),e(".new-action").focus(function(){e(".action-hint").removeClass("hidden")}).blur(function(){e(".action-hint").addClass("hidden")}),e(".action-list").sortable({revert:!0,start:function(t,i){app.dragged=!0,app.draggable_sibling=e(i.item).prev()},stop:function(t,i){app.dragged=!1,app.dropped&&(0==app.draggable_sibling.length&&e(".action-list").prepend(i.item),app.draggable_sibling.after(i.item),app.dropped=!1)},update:function(e,t){app.actions.trigger("updateOrder")}}).disableSelection(),app.actions.fetch({reset:!0}),this.render()},render:function(){var e=app.actions.completed().length,t=app.actions.remaining().length;app.selectedCategoryID||"category-all"==app.selectedCategoryID?(this.$el.show(),this.$statusfilter.html(this.statusTemplate({completed:e,remaining:t})),this.$(".filters li button").removeClass("selected-filter").filter(".filter-"+(app.ActionStatusFilter||"all")).addClass("selected-filter")):this.$el.hide(),this.allCheckbox.checked=!t&&e,this.filterAll(),this.selectAction()},updateOrder:function(){e(".action-list li").each(function(t){for(var i in app.actions.models){var a=app.actions.models[i];a.get("id")===e(this).find("label").data("id")&&a.save({order:t})}})},selectAction:function(){if(e(".actions .selected").removeClass("selected"),app.actions.each(function(e){e.select(!1)}),app.selectedActionID){var t=app.actions.get(app.selectedActionID);t?t.select(!0):app.selectedActionID=""}app.actions.trigger("actionSeletected")},filterOne:function(e){e.trigger("visible")},filterAll:function(){app.actions.each(this.filterOne,this)},setFilterAll:function(){app.AreaRouter.navigate(app.selectedCategoryID+(app.selectedActionID?"/a"+app.selectedActionID:"")+"/fall",{trigger:!0})},setFilterActive:function(){app.AreaRouter.navigate(app.selectedCategoryID+(app.selectedActionID?"/a"+app.selectedActionID:"")+"/factive",{trigger:!0})},setFilterCompleted:function(){app.AreaRouter.navigate(app.selectedCategoryID+(app.selectedActionID?"/a"+app.selectedActionID:"")+"/fcompleted",{trigger:!0})},addOne:function(e){var t=new app.ActionView({model:e});this.$list.append(t.render().el)},addAll:function(){this.$list.html(""),app.actions.each(this.addOne,this)},newAttributes:function(){return{title:this.$input.val().trim(),order:app.actions.nextOrder(),category:app.selectedCategoryID}},createOnEnter:function(t){t.which===ENTER_KEY&&this.$input.val().trim()&&(""===app.selectedAreaID||"area-all"===app.selectedAreaID?(e("#dialog-confirm #dialog-message").html("Please select the area where this activity should be created."),e("#dialog-confirm").dialog({title:"Area must be selected",resizable:!1,height:"auto",width:400,modal:!0,buttons:{OK:function(){e(this).dialog("close")}}})):""===app.selectedCategoryID||"category-all"===app.selectedCategoryID?(e("#dialog-confirm #dialog-message").html("Please select the category where this activity should be created."),e("#dialog-confirm").dialog({title:"Category must be selected",resizable:!1,height:"auto",width:400,modal:!0,buttons:{OK:function(){e(this).dialog("close")}}})):(app.actions.create(this.newAttributes(),{wait:!0}),this.filterAll(),this.$input.val("")))},clearCompleted:function(){return e(function(){e("#dialog-confirm #dialog-message").html("Are you sure you want to permanently clear all completed actions?"),e("#dialog-confirm").dialog({title:"Clear Completed",resizable:!1,height:"auto",width:400,modal:!0,buttons:{"Clear Completed":function(){"category-all"===app.selectedCategoryID?_.invoke(app.actions.completed(),"destroy"):_.invoke(app.actions.completedByCategory(app.selectedCategoryID),"destroy"),app.actions.trigger("toggleCompleted"),e(this).dialog("close")},Cancel:function(){e(this).dialog("close")}}})}),!1},toggleAllComplete:function(){var e=this.allCheckbox.checked;app.actions.each(function(t){t.get("category")===app.selectedCategoryID&&t.save({completed:e})}),app.actions.trigger("toggleCompleted")}})}(jQuery);
var app=app||{};!function(e){"use strict";app.CategoryView=Backbone.View.extend({tagName:"li",template:_.template(e("#category-template").html()),events:{"click label":"select","dblclick label":"edit","click .destroy":"clear","keydown .edit":"handleKeyPress","blur .edit":"close"},initialize:function(){this.listenTo(this.model,"change",this.render),this.listenTo(this.model,"destroy",this.remove),this.listenTo(this.model,"visible",this.toggleVisible)},render:function(){if(void 0===this.model.changed.id)return this.$el.html(this.template(this.model.toJSON())),this.$input=this.$(".edit"),this.$el.addClass("droppable-category draggable-category"),app.selectedCategoryID==this.model.id&&this.$el.addClass("selected"),"category-all"==this.model.id&&this.$el.addClass("category-all"),this},select:function(){e(".editing").removeClass("editing"),app.dragged||app.selectedCategoryID==this.model.id||(this.updateNavigation(this),e(".new-action").focus())},edit:function(){this.$el.hasClass("category-all")||(this.$el.addClass("editing"),this.$input.focus())},close:function(){var e=this.$input.val(),t=e.trim();if(this.$el.hasClass("editing")){if(t===this.model.get("title"))return void this.$el.removeClass("editing");t?this.model.save({title:t}):this.clear(),this.$el.removeClass("editing"),this.updateNavigation(this)}},handleKeyPress:function(e){e.which===ESC_KEY?(this.$el.removeClass("editing"),this.$input.val(this.model.get("title"))):e.which===ENTER_KEY&&this.close()},clear:function(){var t=this.model;e(function(){var i=app.actions.actionsByCategory(t.id);e("#dialog-confirm #dialog-message").html("Are you sure you want to permanently delete this category and "+i.length+" associated action"+(1==i.length?"":"s")+"?"),e("#dialog-confirm").dialog({title:"Delete Category",resizable:!1,height:"auto",width:400,modal:!0,buttons:{"Delete Category":function(){_.invoke(i,"destroy"),app.selectedCategoryID="",t.destroy(),app.actions.trigger("filter"),e(this).dialog("close")},Cancel:function(){e(this).dialog("close")}}})})},updateNavigation:function(e){app.AreaRouter.navigate(e.model.id+(app.selectedActionID?"/a"+app.selectedActionID:"")+"/f"+app.ActionStatusFilter,{trigger:!0})}})}(jQuery);
var app=app||{};!function(e){"use strict";app.CategoryListView=Backbone.View.extend({el:".categories",events:{"keypress .new-category":"createOnEnter"},initialize:function(){this.$input=this.$(".new-category"),this.$footer=this.$(".footer"),this.$list=e(".category-list"),this.$el.removeClass("hidden"),this.allCategory=new app.Category({title:"(All)",id:"category-all",visible:!0}),this.listenTo(app.categories,"add",this.addOne),this.listenTo(app.categories,"reset",this.addAll),this.listenTo(app.categories,"filter",this.render),this.listenTo(app.categories,"destroy",this.render),this.listenTo(app.categories,"updateOrder",this.updateOrder),this.listenTo(app.actions,"reset",this.updateCompleteCountsAllCategory),this.listenTo(app.actions,"update",_.debounce(this.updateCompleteCountsAll,0)),this.listenTo(app.actions,"change:completed",_.debounce(this.updateCompleteCountsAll,0)),e(".new-category").focus(function(){e(".category-hint").removeClass("hidden")}).blur(function(){e(".category-hint").addClass("hidden")}),e(".category-list").sortable({revert:!0,start:function(e,t){app.dragged=!0},stop:function(e,t){app.dragged=!1},update:function(e,t){app.categories.trigger("updateOrder")}}),app.categories.fetch({reset:!0}),this.render()},render:function(){this.filterAll(),this.selectCategory(),this.updateCompleteCountsAllCategory(),this.resetDroppable()},resetDroppable:function(){e(".droppable-category").droppable({accept:".draggable-action",activeClass:"active",hoverClass:"hovered",drop:function(t,a){app.dropped=!0;var i=app.actions.get(e(a.draggable).find(".title-label").data("id"));i.save({category:e(t.target).find(".title-label").data("id")}),app.actions.trigger("filter"),app.actions.trigger("update")}})},updateOrder:function(){e(".category-list li").each(function(t){for(var a in app.categories.models){var i=app.categories.models[a];i.get("id")===e(this).find("label").data("id")&&i.save({order:t})}}),this.resetDroppable()},selectCategory:function(){if(e(".categories .selected").removeClass("selected"),app.categories.each(function(e){e.select(!1)}),app.selectedCategoryID&&app.categories.get(app.selectedCategoryID)||1!==app.categories.length||(app.selectedCategoryID=app.categories.models[0].id,app.actions.trigger("filter")),app.selectedCategoryID)if(app.selectedCategoryID===this.allCategory.id)this.allCategory.set("selected",!0),this.allCategory.trigger("change");else{var t=app.categories.get(app.selectedCategoryID);t?t.select(!0):app.selectedCategoryID=""}},updateCompleteCountsAll:function(){app.categories.each(this.updateCompleteCountsOne,this),this.updateCompleteCountsAllCategory()},updateCompleteCountsOne:function(e){e.save({completedCount:app.actions.completedByCategory(e.id).length,remainingCount:app.actions.remainingByCategory(e.id).length})},updateCompleteCountsAllCategory:function(){this.allCategory.set({completedCount:app.actions.completed().length,remainingCount:app.actions.remaining().length}),this.allCategory.trigger("change")},filterOne:function(e){e.trigger("visible")},filterAll:function(){app.categories.each(this.filterOne,this)},addOne:function(e){var t=new app.CategoryView({model:e});this.$list.append(t.render().el)},addAll:function(){this.$list.html(""),this.addOne(this.allCategory),app.categories.each(this.addOne,this)},newAttributes:function(){return{title:this.$input.val().trim(),order:app.categories.nextOrder()}},createOnEnter:function(e){e.which===ENTER_KEY&&this.$input.val().trim()&&(app.categories.create(this.newAttributes(),{wait:!0}),this.$input.val(""),this.render())}})}(jQuery);
var app=app||{};!function(){"use strict";app.ActionStatusFilter="all";var t=Backbone.Router.extend({routes:{":category(/a:action)(/f:filter)":"setFilter"},setFilter:function(t,e,a){app.selectedCategoryID=t||"",app.selectedActionID=e||"",app.ActionStatusFilter=a||"all",app.categories.trigger("filter"),app.actions.trigger("filter")}});app.AreaRouter=new t,Backbone.history.start()}();
...
function showButtonMenu(e){$("#menu").removeClass("hidden").addClass("menu-show").removeClass("menu-hide"),$("#menu-button").removeClass("menu-button-show").addClass("menu-button-hide"),e.stopPropagation(),e.preventDefault(),$("body").click(hideButtonMenu)}function hideButtonMenu(){$("#menu").addClass("menu-hide").removeClass("menu-show"),$("#menu-button").removeClass("menu-button-hide").addClass("menu-button-show"),$("body").off("click"),console.log("hiding")}var app=app||{},ENTER_KEY=13,ESC_KEY=27;app.dragged=!1,app.dropped=!1,app.draggable_sibling,$(function(){"use strict";new app.CategoryListView,new app.ActionListView,new app.ActionDetailsView}),$(document).ready(function(){$("#menu-button").click(showButtonMenu),$("#menu").mouseleave(hideButtonMenu),$("#help-button").click(function(){$("#dialog-help").dialog({title:"Todo - Help",resizable:!0,height:"auto",width:"auto",modal:!1,buttons:{OK:function(){$(this).dialog("close")}}})}),$("#download-button").click(function(){var e=JSON.stringify(localStorage),t=new File([e],"todo.json",{type:"text/plain;charset=utf-8"});saveAs(t)}),$("#upload-button").click(function(){$("#dialog-confirm #dialog-message").html('Importing data will <strong>permanently</strong> delete exising data. Continue?<br><div id="import-status"></div><input type="file" id="import-file">'),$("#dialog-confirm").dialog({title:"Import Data",resizable:!1,height:"auto",width:400,modal:!0,buttons:{"Overwrite and Import":function(){var e=document.getElementById("import-file").files[0];if(e){var t=new FileReader,o=$(this);t.onload=function(e){var t=JSON.parse(e.target.result);localStorage.clear();for(var a in t)localStorage.setItem(a,t[a]);app.selectedCategoryID="",app.categories.fetch(),app.actions.fetch(),app.actions.trigger("filter"),o.dialog("close")},t.readAsText(e)}else document.getElementById("import-status").innerHTML="No file selected, please select a file."},Cancel:function(){$(this).dialog("close")}}})}),$("#clear-button").click(function(){$("#dialog-confirm #dialog-message").html("Are you sure you want to <strong>permanently</strong> delete category and action data?"),$("#dialog-confirm").dialog({title:"Delete All Data",resizable:!1,height:"auto",width:400,modal:!0,buttons:{"Delete Everything":function(){localStorage.clear(),app.selectedCategoryID="",app.categories.reset(),app.actions.reset(),app.actions.trigger("filter"),$(this).dialog("close")},Cancel:function(){$(this).dialog("close")}}})})});